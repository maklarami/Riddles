...
class Part:
    ...
    def __init__(self, part_definition: str):
        self.props :dict = {}
        elements = part_definition[1:-1].split(",")
        for element in elements:
            self.props[element[0]] = int(element[2:])
...
class Step:
    def __init__(self, step: str):
        logic = step.split(":")[0]
        self.result = step.split(":")[1]
        property = logic[0]
        operator = logic[1]
        value = int(logic[2:])

        if operator == ">":
            self.f = lambda part: part.props[property] > value
        elif operator == "<":
            self.f = lambda part: part.props[property] < value
        else:
            raise Exception()

    def run(self, part: Part) -> Union[str, None]:
        if self.f(part):
            return self.result
        else:
            return None
...
class Workflow:
    def __init__(self, config: str):
        steps_config = config.split(",")
        self.default = steps_config.pop()
        self.steps = [Step(step_config) for step_config in steps_config]

    def run(self, part : Part) -> str:
        for step in self.steps:
            result = step.run(part)
            if result:
                return result
...
def main(input : str) -> int:
    ...
    workflows_dict: dict[str, Workflow] = {}    # Initialize an empty dictionary to hold workflow objects
    parts_list: list[Part] = []                 # Initialize an empty list to hold part objects}
    ...
    for line in input.splitlines():
       if not line:
            ...
            parts_list.append(Part(line))
       else:
            seg = line.split("{")
            workflows_dict[seg[0]] = Workflow(seg[1][:-1])

    for part in parts_list:
        workflow_name = "in"

        while True:
            result = workflows_dict[workflow_name].run(part)
            if result == "A":
    ...
    print(f"Number of accepted parts: {len(accepted_parts)}")

    mysum = sum([part.sum() for part in accepted_parts])
    print(mysum)
    return mysum

if __name__ == "__main__":
    main("""qdg{a>3896:kg,s>540:tjk,s>478:A,A}
